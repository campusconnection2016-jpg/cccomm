...
const handleReview = () => {
  ...
  setReviewMap((prev) => ({ ...prev, [qId]: true }));
...
useEffect(() => {
  if (questions && questions.length > 0) {
    ...
    questions.forEach((q, index) => {
      ...
      console.log("Option E:", q.option_e);
...
const [optionMarks, setOptionMarks] = useState({});
 ...
 const handleOptionSelect = async (opt) => {
  ...
  readOnly
  onClick={() => {
    if (!reviewedQuestions[questions[currentQuestionIndex].id]) {
...
/>
            ...
            {reviewMap[questions[currentQuestionIndex].id] && (
  <div style={{ marginTop: "10px", color: "white" }}>
    Correct Answer:{" "}
    {
      questions[currentQuestionIndex][
        `option_${questions[currentQuestionIndex].answer.toLowerCase()}`
      ]
    }
  </div>
)}
  ...
  {/* REVIEW — center (NEW, no CSS change) */}
...
<button
  ...
  disabled={
    !answers[questions[currentQuestionIndex].id] || // no option selected
    reviewedQuestions[questions[currentQuestionIndex].id] // already reviewed
  }
>
  Review
</button>
  ...
  {/* NEXT / FINISH — right (same CSS, just conditional) */}
