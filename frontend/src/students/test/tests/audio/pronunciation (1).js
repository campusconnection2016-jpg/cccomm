...
const evaluatePronunciation = async (spoken, qIndex = currentQuestionIndex) => {
  ...
  // 🚫 DO NOT highlight or modify question text for bilingual skills
  if (ALLOWED_SKILL_TYPES.includes(skillType)) {
    setHighlightedText(""); // keep original translated question
  } else {
    const clean = (text) =>
      text
        .replace(/[.,!?;:()'"-]/g, "")
    ...
    const correctWords = clean(q.question_text).split(" ");
...
useEffect(() => {
  ...
  logSkillTypeQuestionApi(skillType, currentQ.question_text)
    .then((res) => {
      const t = res?.data?.translated_text || null;
      setTranslatedText(t);
    })
    .catch(() => {
      setTranslatedText(null);
...
const displayQuestionText = ALLOWED_SKILL_TYPES.includes(skillType)
  ? translatedText // ❌ do NOT fallback to English
...
>
  {ALLOWED_SKILL_TYPES.includes(skillType) && isTranslating ? (
    <span style={{ color: "#ccc" }}>Loading question…</span>
  ) : (
    <div
      dangerouslySetInnerHTML={{
        __html: highlightedText || `<span>${displayQuestionText}</span>`,
      }}
    />
  )}
</div>
