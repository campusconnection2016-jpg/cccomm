...
const [skillType, setSkillType] = useState(null);
 ...
 console.log("➡ Sub Topic (from payload):", payload?.sub_topic);
...
useEffect(() => {
  ...
  getSkillTypeByTestNameApi(selectedTest.test_name)
    .then((res) => {
      if (res?.skill_type) {
        console.log("✅ Skill Type (from API):", res.skill_type);
        setSkillType(res.skill_type);
      } else {
        console.warn("⚠️ Skill Type missing from API, using sub_topic as skill type");
        console.log("➡ Sub Topic used as skill type:", payload?.sub_topic);
        setSkillType(payload?.sub_topic); // ✅ FALLBACK
      }
    })
    .catch((err) => {
      console.error("❌ Skill type fetch failed, using sub_topic:", err);
      console.log("➡ Sub Topic used as skill type:", payload?.sub_topic);
      setSkillType(payload?.sub_topic); // ✅ FALLBACK
    });
      ...
      navigate(path, {
        state: {
          ...
          mapping_id: mappingId,
           ...
           communication_category: "PracticeTest", // ✅ ADD HERE
