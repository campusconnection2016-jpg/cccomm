...
const [filtered, setFilteredDatas] = useState([]);
  ...
  .then(data => {
    ...
    setFilteredDatas(sorted); // if using filteredData
  })
...
const sortByRegNo = () => {
 ...
 const exportToExcel = async () => {
  ...
  const dataToExport = filterCandidates().filter(c =>
    selectedStudents.has(c.student_id)
  );

  const filteredData = dataToExport.map(({
  ...
  wsData.forEach(row => worksheet.addRow(Object.values(row)));
