...
const Uploadstudent = ({ collegeName, username, onUploadComplete  }) => {
  ...
  const [loginData, setLoginData] = useState({ user_name: '', password: '' }); // State to store login details


  useEffect(() => {
    const fetchLoginData = async () => {
      console.log("Fetching login data for username:", username); // Debug username
      try {
        const loginResponse = await LoginpassApi(username); // Fetch login details based on user_name
        console.log("API response:", loginResponse); // Log the full API response to see its structure
  
        // Check if the response has the expected fields
        if (loginResponse && loginResponse.user_name && loginResponse.password) {
          console.log("Valid response received. Setting login data.");
          setLoginData({
            user_name: loginResponse.user_name,
            password: loginResponse.password,
          });
        } else {
          console.log("Invalid response structure or missing fields.");
          console.log("Expected user_name and password, got:", loginResponse); 
          setShowError(true);
        ...
        setShowError(true);
        setErrorMessage('Error fetching login data');
      }
    };
  ...
  const handleInputpassChange = (field, value) => {
    console.log(`Changing ${field} to`, value); // Debug input changes
    setLoginData((prevData) => ({
      ...prevData,
      [field]: value, // Update local state when input changes
    }));
  };
  
  // Log updated loginData state whenever it changes
  useEffect(() => {
    console.log("Updated loginData state:", loginData); // Debug state after change
  }, [loginData]);
  
  useEffect(() => {
        if (username) {
          getTestcandidateApi(username)
            .then(data => {
              if (data.length > 0) {
                setTestCandidates(data[0]); // Backend already filters, return only relevant data
                setTcID(data[0].id);
  ...
  const handleFileChange = (index, field, file) => {
    ...
    updatedStudents[index] = {
      ...updatedStudents[index],
      [field]: file, // Store the selected file object
    };
    setStudents(updatedStudents);
  };
  ...
  if (registrationNumber === undefined || registrationNumber === null || registrationNumber.trim() === '' || parseFloat(registrationNumber) === 0) {
    ...
    try {
        ...
        const loginResponse = await LoginpassApi(student.user_name); // Pass student user_name if available
        ({ user_name, password } = loginResponse); // Destructure the response
        console.log('Fetched login credentials:', { user_name, password });
    } catch (error) {
        console.error('Error fetching login data:', error);
        setErrorMessage('Failed to fetch login credentials');
        setShowError(true);
        return; // Exit early if login credentials fetch failed
    }
    ...
    try {
      ...
      formData.append("students_name", student.students_name);
      formData.append("registration_number", student.registration_number);
      formData.append("email_id", student.email_id);
      formData.append("mobile_number", student.mobile_number);
    
      
      try {
        // Now, update the login credentials with the updated user_name and password
       const loginUpdateData = {
  user_name: loginData.user_name,
  password: loginData.password,
  email_id: student.email_id,        // include updated email
  mobile_number: student.mobile_number // include updated mobile number
};
console.log("loginUpdateData",loginUpdateData)
await updateLoginPasswordApi(user_name, loginUpdateData);
 // Update the login credentials via API
                ...
                <Row md={12}>
                  ...
                  <Col>
                    <div controlId='marks_12th' className='ranges-over'>
                      <label htmlFor="marks12th">12th/Diploma Marks (%)<span style={{ color: '#F1A128' }}>**</span></label><p></p>
                      <input
                        type="range"
                        id="marks12th"
                        name="marks12th"
  ...
  trainee.mobile_number && trainee.mobile_number !== 0 && trainee.mobile_number !== "0.0"
    ...
    : ""
                  ...
                  <Col >
                    <div controlId='skill_id' className='skill-can'>
            ...
            <div controlId='user_name' className='add-profile'>
              ...
              <input
                ...
                readOnly
                //onChange={(e) => handleInputpassChange('user_name', e.target.value)}
              />
            </div>
          </Col>
                </Row>
  ...
  />
     ...
     {/*}   <Col>
  <div controlId='resume' className='add-profile'>
    <label className='label5-ques'>Upload Resume (PDF/DOC/DOCX)</label><p></p>
    <input
      type="file"
      accept=".pdf,.doc,.docx"
      className='input-ques-text-po'
      onChange={(e) => handleFileChange(index, 'resume', e.target.files[0])}
    />
  </div>
</Col>
<Col>
  <div controlId='photo' className='add-profile'>
    <label className='label5-ques'>Upload Photo (JPG/PNG)</label><p></p>
    <input
      type="file"
      accept="image/*"
      className='input-ques-text-po'
