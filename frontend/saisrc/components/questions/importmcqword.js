...
const ImportMCQWord = ({ isFormValid, formData, selectedFolderName, collegeName, userRole,selectedTopic }) => {
  ...
  const onDrop = useCallback((acceptedFiles) => {
    if (acceptedFiles.length > 0) {
      ...
      if (fileType === 'docx' || fileType === 'pdf') {
        ...
        setErrorMessage('Only Word (.docx) or PDF (.pdf) files are allowed.'); // Set error message
        setShowError(true);
      }
    }
  }, []);
    ...
    try {
      ...
      const apiFunction =
        userRole === 'Placement Officer'
          ? () => WordImportMCQ_Api_place(formDataToSend)
          : () => WordImportMCQ_Api(formDataToSend);
